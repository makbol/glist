...
//        });
    ...
    protected boolean onStartNewGameExecuted(WebSocket socket, Player player, BaseCommand command) {
        if (command instanceof StartNewGameCommand) {
            try {
                room.setGameEventHandler(gameHandler);
                room.startNewGame();
    ...
    protected void beginNextRound() {
        room.getGame().preparePlayerForAnotherGreatRoundOfOurSuperbInteractiveScalableMultiplayerSuperfragaliciousGameOfTron();
    }
